...
/// Conway's Game of Life on a toroidal grid. Pure value type — the board owns one and redraws it.
///
/// Ported from ios/Conway/Life.swift, function-for-function. No resizing here: a watch face
/// doesn't rotate or split like an iPad, so the grid is a fixed size chosen to stay legible
/// (see `Board.gridSize`) rather than tracking the view's geometry.
...
/// Small patterns only — sized to actually fit `Board.gridSize` (12x12) with room to breathe,
/// unlike the iOS set which also carries a 9x38 Gosper gun.
