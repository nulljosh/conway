...
// and MCP surfaces cannot describe different behaviour. That module also carries the board
// and step ceilings — this endpoint is public and runs a simulation per request.
  ...
  switch (method) {
    case 'initialize':
      return reply({
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'conway', version: VERSION },
      });
    ...
    case 'tools/call': {
      ...
      try {
        ...
        if (err instanceof ToolError) {
          return reply({ content: [{ type: 'text', text: `conway: ${err.message}` }], isError: true });
        }
        throw err;
      }
      if (!result) {
